...
function Puzzlez() {
    ...
    const toogle = (e,num) =>{
        ...
        if (lock) {
            return 0; // Exit the function if a win has been detected
        }
        if (count%2===0){
    ...
    const won = (winner) => {
        ...
        if(winner==="x"){
            titleRef.current.innerHTML = `Wow : <img src=${cross_icon} /> Wins`
        }else{
            titleRef.current.innerHTML = `Wow: <img src=${circle_icon} /> Wins`
        }
    ...
    const reset = ()=> {
        ...
        data = ["","","","","","","","",""];
        titleRef.current.innerHTML = 'Cross Puzzle'
        // box_array.map((box)=>{
  ...
  return (
        ...
        <div className="conatiner">
            <h1 className="title" ref={titleRef}>Cross Puzzle </h1>
